          ...
          {list.map((item, i) => (
            <li key={i} className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{item.id}</p>
                <p className="text-[11px] text-slate-500 mt-1">Due: {item.due}</p>
              </div>
              <div className="shrink-0 text-right">
                ...
                <span
                  className={
                    item.status === 'Overdue'
                      ? 'inline-flex mt-1 text-[11px] px-2 py-0.5 rounded-full bg-red-100 text-red-900 border border-red-200 uppercase tracking-wider font-medium'
                      : 'inline-flex mt-1 text-[11px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-900 border border-blue-200 uppercase tracking-wider font-medium'
                  }
                >
                  {item.status}
                </span>
