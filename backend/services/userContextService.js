...
// Build context for a specific category
async function buildCategoryContext(email, category) {
  if (!email) {
    return { context: "No logged-in user email was provided.", sources: [] };
  }
  if (!category) {
    return { context: "Category not specified.", sources: [] };
  }
  try {
    const policies = await Policy.find({ "insured.email": email });
    const policyNumbers = policies.map(p => p.policyNumber).filter(Boolean);
    ...
    if (category === "policies") {
      const policySection = policies.length
        ? policies
            .map((p, i) => `${i + 1}. Policy: ${p.policyNumber}, Type: ${p.policyType || "N/A"}, Status: ${p.status || "N/A"}\n   - Address: ${p.propertyAddress ? `${p.propertyAddress.addressLine1}, ${p.propertyAddress.city}, ${p.propertyAddress.state} ${p.propertyAddress.zipCode}` : "N/A"}\n   - Premium: ${formatMoney(p.annualPremium)}, Agent: ${p.agent?.name || "N/A"}\n   - Coverages: ${p.coverages?.length ? p.coverages.map(c => `${c.name} (${formatMoney(c.limit)})`).join(", ") : "None"}\n   - Documents: ${p.documents?.length ? p.documents.map(d => d.name || d.documentName || d.fileName || "Doc").join(", ") : "None"}\n   - History: ${p.transactionHistory?.length ? p.transactionHistory.map(t => `${t.transactionType || t.type} (${t.transactionDate || t.date || "N/A"})`).join(", ") : "None"}`)
            .join("\n")
        : "No policies found.";
      context = `User Policies:\n${policySection}`;
      sources = [{ title: "Policies", type: "mongodb" }];
    } else if (category === "claims") {
      const claimSection = claims.length
        ? claims
            .map((c, i) => `${i + 1}. Claim: ${c.ClaimNumber}, Status: ${c.Status}, Loss Date: ${c.LossDate || "N/A"}\n   - Description: ${c.DescriptionOfLoss || "N/A"}\n   - Paid: ${formatMoney(c.PaidLoss)}`)
            .join("\n")
        : "No claims found.";
      context = `User Claims:\n${claimSection}`;
      sources = [{ title: "Claims", type: "mongodb" }];
      ...
      sources = [{ title: "Billing", type: "mongodb" }];
    } else {
      // "others" – return full user context
      const fullContext = await buildUserContext(email);
      return { context: fullContext, sources: [{ title: "Full User Data", type: "mongodb" }] };
    }

    if (!context) {
      context = "No information found for the requested category.";
    }

    return { context, sources };
  } catch (error) {
    console.error("Error building category context:", error);
    return { context: "Error retrieving data.", sources: [] };
  }
}
